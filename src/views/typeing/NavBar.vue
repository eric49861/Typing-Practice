      ...
      <div>
        ...
        <div class="selectContent-wrapper">
          ...
          <div class="selectContent" ref="selectContent">
            <div class="selectContent-item" v-for="content in ContentList" :key="content.id"
      ...
      <div class="addContent">
        ...
        <div class="form">
          ...
          <div class="language">
            ...
            英文<input type="radio" name="language" value="english">
          </div>
          <div>
            <div>正文</div>
...
nav {
  ...
  .nav-right {
    ...
    .select-content-btn:hover+.selectContent-wrapper,
    .selectContent-wrapper:hover {
      opacity: 1;
      max-height: 60vh;
    }
  }
...
const getContentList = (language) => {
  ...
  for (let key in contentInfo) {
    const { id, title, author, dynasty, language, con } = contentInfo[key];
    ContentList.push({ id, title, author, dynasty, language, con });
  }
  return ContentList;
}
// 头部导航栏/添加内容
const useNavEffect = (router, warningHandleClick, language, ContentList) => {
  ...
  const addContent = () => {
    ...
    const id = Object.keys(JSON.parse(localStorage.getItem('ContentInfoCN'))).length;
